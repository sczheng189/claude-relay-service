...
const authenticateApiKey = async (req, res, next) => {
  ...
  try {
    ...
    // 检查 Claude 周费用限制（字段名沿用 weeklyOpusCost*，变量名使用 Claude 语义）
    const weeklyClaudeCostLimit = validation.keyData.weeklyOpusCostLimit || 0
    if (weeklyClaudeCostLimit > 0) {
      ...
      if (isClaudeFamilyModel(model)) {
        const weeklyClaudeCost = validation.keyData.weeklyOpusCost || 0
